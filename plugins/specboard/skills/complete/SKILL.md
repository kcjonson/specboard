...
- `list_projects`, then `get_items(project, { item_key: epic_key, include_children: true })` to load
...
- **File the followups the session surfaced.** Anything you flagged for later (a discovered bug, a
  ...
  conversation: `create_item(project, title, 'bug' | 'task' | 'epic', { description })`, or
  `create_items(project, other_epic_key, [...])` for tasks under another epic. If it isn't on the
  board, it's lost.
- **Move deferred tasks out of this epic.** A task that was punted (out of scope, next phase, not
  ...
  it from this epic with `delete_item(project, task_key, 'task')`. The work survives in its new
...
- `update_item(project, epic_key, 'epic', { sub_status: 'complete' })` sets the board to `done`.
- Add a closing note summarizing what shipped, plus what was deferred and where it went:
  `update_item(project, epic_key, 'epic', { note: '<what shipped; deferred X to #Y>' })`.
