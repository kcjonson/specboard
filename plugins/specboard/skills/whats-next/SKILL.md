...
Call `list_projects` to find the user's project(s) and their refs. A project is addressed as
`owner/project` (e.g. `acme/roadmap`): the owner's user slug, then the project's slug. Each
project in the response carries `owner`, `slug`, and the combined `ref`.
...
- Store the resolved `ref` and pass it as `project` on every subsequent call (omit it entirely when
  the repo is bound).

A bound repo's `.mcp.json` carries `owner/project` in the `X-Specboard-Project` header; the MCP
server scopes `list_projects` and the item tools to that project (still gated per user by the
access check). A bare slug (`roadmap`) also works, but it means *the caller's own* project of
that slug, so anyone else on the repo gets "not found": bind the full `owner/project` in any repo
with collaborators. The ref is a shared, non-secret reference, committing it grants nothing
without each user authenticating individually.
...
### 2c. MCP State
- `get_items(project, { status: 'in_progress', include_children: true, include_notes: true })`
- `get_items(project, { status: 'in_review', include_children: true })`
- `get_items(project, { status: 'ready' })`
...
`get_items(project, { item_key: epic_key, include_children: true, include_notes: true })` returns the
...
2. `update_item(project, epic_key, 'epic', { branch_name: '<branch>', sub_status: 'scoping' })`
3. `create_items(project, epic_key, items)`, turn the plan's steps into tasks.
4. `update_item(project, epic_key, 'epic', { sub_status: 'in_development' })` when you start coding.
5. Start the first task: `update_item(project, task_key, 'task', { status: 'in_progress' })`.

You may also create the epic itself when one doesn't exist for work you're about to do:
`create_item(project, title, 'epic', { specs: [...] })`. Create standalone tasks and bugs the same
...
- Start: `update_item(project, task_key, 'task', { status: 'in_progress' })`
- Complete: `update_item(project, task_key, 'task', { status: 'done', note: 'Added Redis session store' })`
- Block: `update_item(project, task_key, 'task', { status: 'blocked', note: 'Need auth spec clarification' })`
- Unblock: `update_item(project, task_key, 'task', { status: 'ready' })`
...
epic-level note at real decision points: `update_item(project, epic_key, 'epic', { note: '...' })`.
