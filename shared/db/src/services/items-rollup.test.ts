 ...
 * how far the walk goes, and whose updated_at a child write moves. The mocked suite in
...
import { createItem, createItems, deleteItem, moveItem, startItem, blockItem, updateItem } from './items.ts';
...
describe('the walk', () => {
	it('keeps climbing past a level that holds still: an explicit in_progress parent under a ready grandparent', async () => {
		await insertItem(1, null, 'ready', 'explicit');
...
describe('updated_at on a child write', () => {
	beforeEach(async () => {
		...
		await insertItem(2, 1, 'ready', 'explicit');
		await insertItem(3, 2, 'ready', 'explicit');
	});
	...
	it('creating a child touches its parent, leaves its status and source alone, and not the grandparent', async () => {
		...
		expect(await item(2)).toEqual({ status: 'ready', status_source: 'explicit', touched: true });
