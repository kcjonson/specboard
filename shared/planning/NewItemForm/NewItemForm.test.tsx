...
function createButton(container: Element): HTMLButtonElement {
	...
	if (!(button instanceof HTMLButtonElement)) {
		throw new Error('No create button in the rendered NewItemForm');
	}
	return button;
}
...
describe('NewItemForm', () => {
	it('emits the drafted title, description, status and type', () => {
		...
		const { container } = render(<NewItemForm createType="bug" onCreate={onCreate} />);
	...
	it('passes a parentKey through when one was supplied', () => {
		const onCreate = vi.fn();
		const { container } = render(<NewItemForm createType="task" parentKey="SB-7" onCreate={onCreate} />);
	...
	it('omits parentKey entirely when there is no parent', () => {
		const onCreate = vi.fn();
		const { container } = render(<NewItemForm onCreate={onCreate} />);
	...
	it('offers only the statuses a brand new item can be in', () => {
		const { container } = render(<NewItemForm onCreate={vi.fn()} />);
