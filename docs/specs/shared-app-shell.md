...
**Deep links.** OAuth callbacks, magic links, and item links (canonical form `/projects/:owner/:project/items/:key`, see [kanban-ui.md](kanban-ui.md#item-urls)) all need to reach a running app: custom URL scheme plus universal links on iOS, app links on Android, protocol handler on desktop.
