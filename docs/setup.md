...
The folder routes that attach such a directory to a project (`POST`/`DELETE /api/projects/:slug/folders`) are only registered when the API starts with `LOCAL_STORAGE_ENABLED=true`. `docker-compose.yml` sets it; the cloud task definitions do not, so those routes 404 on staging and production.
