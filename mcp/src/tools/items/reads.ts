...
export async function getItems(project: ResolvedProject, args: Record<string, unknown>): Promise<ToolResult> {
	...
	if (args.item_key !== undefined && args.item_key !== null) {
		...
		if (parsed === null) {
			return {
				content: [{ type: 'text', text: `${String(args.item_key)} is not an item key in project ${formatProjectRef(project.ownerSlug, project.slug)} (its items look like ${project.key}-1).` }],
				isError: true,
			};
		}
		itemNumber = parsed;
