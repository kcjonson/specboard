...
const SERVER_INSTRUCTIONS = `You are connected to Specboard, the user's planning board: epics, chores, bugs, and their tasks. Use these tools whenever the user is planning, picking up work, or tracking development status.

Tools: list_projects finds the project (a repo bound via .mcp.json X-Specboard-Project auto-selects one). get_items reads work by status (ready/in_progress/in_review/done), by type, by search, or one item by item_id with include_tasks/include_notes. create_item makes an epic/chore/bug, or a task under parent_id; create_items bulk-creates tasks. update_item changes title/description/status/sub_status/notes/branch_name/pr_url. Setting an epic's sub_status drives the board: scoping/in_development/pr_open -> in_progress, complete -> done.
